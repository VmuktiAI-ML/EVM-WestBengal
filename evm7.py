...
PERSON_EVM_OVERLAP_IOP_THRESH = 0.20

EVM_ROI_EXPAND_LR_SCALE  = 0.05
EVM_ROI_EXPAND_TOP_SCALE = 0.05
DEAD_ZONE_SCALE          = 0.35
DEAD_ZONE_OVERLAP_THRESH = 0.20 
PROXIMITY_BOOTH_SCALE    = 0.45
PERSON_PERSON_IOP_THRESH = 0.50
BOOTH_SAFETY_BUFFER_LR_SCALE = 0.05
BOOTH_SAFETY_BUFFER_TB_SCALE = 0.05
...
# Staff Masking Constants
STAFF_GRID_SIZE         = 40
STAFF_LEARNING_FRAMES   = 10
STAFF_TOTAL_CYCLES      = 60
STAFF_OCCUPANCY_THRESH  = 5

# Voter / officer classifier
HEAD_CLEARANCE_VOTER_THRESH   = 0.50
HEAD_CLEARANCE_OFFICER_THRESH = 0.20
VOTER_OFFICER_LEARNING_FRAMES = 1
VOTER_AREA_PERCENTILE         = 70
...
def get_grid_coords(box, frame_width, frame_height):
    """Maps person feet (bottom-center) to grid coordinates."""
    cx = (box[0] + box[2]) / 2.0
    cy = box[3]
    gx = int((cx / frame_width) * STAFF_GRID_SIZE)
    gy = int((cy / frame_height) * STAFF_GRID_SIZE)
    gx = max(0, min(gx, STAFF_GRID_SIZE - 1))
    gy = max(0, min(gy, STAFF_GRID_SIZE - 1))
    return gx, gy
...
def classify_person(person_box, booth_raw, voter_area_threshold):
    px1, py1, px2, py2 = person_box
    w  = max(1.0, float(px2 - px1))
    h  = max(1.0, float(py2 - py1))
    ar = h / w
    area = w * h

    # Feature 1: aspect ratio
    ar_is_voter   = ar >= 1.50
    ar_is_officer = ar <= 1.25
    ar_clear = ar_is_voter or ar_is_officer  # ambiguous if between 1.25-1.50

    # Feature 2: area relative to EVM — needs booth detected
    if booth_raw is not None:
        bx1, by1, bx2, by2 = booth_raw
        evm_area = max(1.0, (bx2 - bx1) * (by2 - by1))
        rel_area = area / evm_area
        area_is_voter   = rel_area >= 1.5
        area_is_officer = rel_area <= 0.8
        area_clear = area_is_voter or area_is_officer
    elif voter_area_threshold is not None:
        area_is_voter   = area >= voter_area_threshold
        area_is_officer = area < voter_area_threshold
        area_clear = True
    else:
        area_clear = False  # threshold not learned yet — feature unavailable
        area_is_voter = area_is_officer = False

    # Feature 3: head clearance — only valid when head is above EVM top
    if booth_raw is not None:
        _, by1, _, by2 = booth_raw
        evm_h = max(1.0, float(by2 - by1))
        rel = (by1 - py1) / evm_h
        if rel > 0:  # head is above EVM top — signal valid
            head_is_voter   = rel >  HEAD_CLEARANCE_VOTER_THRESH
            head_is_officer = rel <  HEAD_CLEARANCE_OFFICER_THRESH
            head_clear = head_is_voter or head_is_officer
        else:
            head_clear = False  # head not above EVM — feature unavailable
            head_is_voter = head_is_officer = False
    else:
        head_clear = False
        head_is_voter = head_is_officer = False

    # ALL THREE must be available AND all three must agree
    all_available = ar_clear and area_clear and head_clear
    all_voter     = ar_is_voter   and area_is_voter   and head_is_voter
    all_officer   = ar_is_officer and area_is_officer and head_is_officer

    if all_available and all_voter:
        return True,  "VOTER"
    if all_available and all_officer:
        return False, "OFFICER"

    # If not all three conditions satisfied — do not classify, treat as VOTER
    # so the system errs on the side of caution (better to over-alert than miss)
    # print(f"DEBUG: Defaulted to Person -> ar_clear:{ar_clear} area_clear:{area_clear} head_clear:{head_clear} | all_voter:{all_voter} all_officer:{all_officer}")
    return True, "Person"
...
def check_breach(person_dets: sv.Detections, booth_raw,
                 ...
                 staff_mask=None, person_classifications=None):
    overlap_info = []
    ...
    roi_w = ebx2 - ebx1; roi_h = eby2 - eby1
    radius_px = max(evm_w, evm_h) * PROXIMITY_BOOTH_SCALE
    buffer_lr_px = roi_w * BOOTH_SAFETY_BUFFER_LR_SCALE
    buffer_tb_px = roi_h * BOOTH_SAFETY_BUFFER_TB_SCALE

    primary_idx = None
    best_iop = -1.0
    for i, row in enumerate(person_dets.xyxy):
        if person_classifications is not None and not person_classifications[i]['is_voter']:
            continue
        if staff_mask is not None:
            gx, gy = get_grid_coords(row, frame_width, frame_height)
            if staff_mask[gy, gx] == 1:
                continue
    ...
    if primary_idx is None:
        return (0, [], [], None, [], 0.0, booth_expanded, dead_zone, (buffer_lr_px, buffer_tb_px))
    ...
    for i, row in enumerate(person_dets.xyxy):
        ...
        # NEW: Safety Buffer Zone - ignore people whose centroid is outside the expanded ROI
        cx, cy = (row[0] + row[2]) / 2.0, (row[1] + row[3]) / 2.0
        is_near_booth_buffer = (ebx1 - buffer_lr_px <= cx <= ebx2 + buffer_lr_px) and \
                               (eby1 - buffer_tb_px <= cy <= eby2 + buffer_tb_px)
        
        is_safe = is_safe_in_dz or (not is_near_booth_buffer)
        ...
        # LOGIC: If the person is in a safe zone (Dead Zone or Outside Buffer), IGNORE them
        if is_safe:
        ...
        if staff_mask is not None:
            gx, gy = get_grid_coords(row, frame_width, frame_height)
            if staff_mask[gy, gx] == 1:
                overlap_info.append({"iou": 0, "iop": 0, "dead_zone_skip": True, "is_staff": True})
                continue

        if person_classifications is not None and not person_classifications[i]['is_voter']:
            overlap_info.append({"iou": 0, "iop": 0, "dead_zone_skip": True, "reason": "officer"})
            continue
    ...
    return (behind_count, proximity_indices, overlap_info,
            primary_idx, secondary_info, radius_px, booth_expanded, dead_zone, (buffer_lr_px, buffer_tb_px))
...
def save_alert_image(cam_id, frame, timestamp, alert_data,
    ...
    try:
        ...
        if save_local or SAVE_ALERT_IMAGES:
            local_dir = os.path.join(ALERT_FOLDER, alert_type)
...
def draw_debug_overlay(ann, booth_raw, booth_expanded, dead_zone,
                       ...
                       overlap_info, radius_px, buffer_px=0.0):
    ...
    if booth_raw is not None and buffer_px is not None:
        if isinstance(buffer_px, tuple):
            blr, btb = buffer_px
        else:
            blr = btb = buffer_px
            
        ex1, ey1, ex2, ey2 = booth_expanded
        sx1, sy1 = int(ex1 - blr), int(ey1 - btb)
        sx2, sy2 = int(ex2 + blr), int(ey2 + btb)
        _draw_dashed_rect(ann, sx1, sy1, sx2, sy2, (0, 255, 0), gap=10)
        cv2.putText(ann, "SAFETY BUFFER (ROI)", (sx1 + 2, sy2 - 4), font, 0.42, (0, 255, 0), 1, cv2.LINE_AA)
    if dead_zone:
...
def annotate_frame(frame, booth_raw, person_dets: sv.Detections,
                   ...
                   radius_px: float = 0.0, buffer_px=None,
                   vacant_active: bool = False, max_person_active: bool = False,
                   proximity_indices=[],
                   voter_count: int = None, is_calibrating: bool = False,
                   staff_indices=[], person_classifications=None):
    ann = frame.copy()
    h, w = ann.shape[:2]
    ...
    if debug and person_dets is not None and len(person_dets) > 0:
        ann = draw_debug_overlay(ann, booth_raw, booth_expanded, dead_zone,
                                 ...
                                 overlap_info or [], radius_px, buffer_px)

    if person_dets is not None:
        for i, box in enumerate(person_dets.xyxy):
            x1, y1, x2, y2 = map(int, box)
            is_in_breach = i in proximity_indices
            is_staff     = i in staff_indices
            
            pc = person_classifications[i] if person_classifications and i < len(person_classifications) else {}
            label = pc.get('label', 'PERSON')

            if max_person_active:
                color = (0, 200, 0) # Green for max person alert
            elif label == "LEARNING":
                color = (128, 128, 128)
            elif debug and is_staff:
            ...
            # Show the label (VOTER / OFFICER / LEARNING / Person) only in debug mode
            if debug:
                cv2.putText(ann, label, (x1, max(15, y1 - 5)), _FONT, 0.5, color, 1, cv2.LINE_AA)
    ...
    if not vacant_active:
        ...
        cv2.rectangle(ann, (hx1, ry1), (hx2, ry2), color, -1)
        cv2.putText(ann, badge, (hx1 + (HUD_WIDTH - bw) // 2, ry1 + bh + 7),
                    _FONT, font_scale, (0, 0, 0) if color == (210, 210, 210) else (255, 255, 255), 1, cv2.LINE_AA)
        current_y = ry2
...
def process_single_frame_v3(
    ...
    # ── Area learning phase ───────────────────────────────────────────────────
    is_learning_areas = False
    if cam_state is not None:
        if cam_state.area_learning_frame_count < VOTER_OFFICER_LEARNING_FRAMES:
            is_learning_areas = True
            if total_persons > 0:
                for box in person_dets.xyxy:
                    a = (box[2] - box[0]) * (box[3] - box[1])
                    cam_state.collected_areas.append(float(a))
                cam_state.area_learning_frame_count += 1
                if cam_state.area_learning_frame_count == VOTER_OFFICER_LEARNING_FRAMES:
                    if cam_state.collected_areas:
                        cam_state.voter_area_threshold = float(
                            np.percentile(cam_state.collected_areas, VOTER_AREA_PERCENTILE))
                        log.info(f"[AREA-LEARN] Threshold: {cam_state.voter_area_threshold:.0f}px²")
                    else:
                        cam_state.voter_area_threshold = 8000.0
                        log.warning("[AREA-LEARN] No samples — using fallback 8000px²")

    # ── Classify each person as VOTER or OFFICER ──────────────────────────────
    # Pre-compute EVM ROI for centroid override (person inside ROI is always VOTER)
    _evm_roi = None
    if booth_raw is not None:
        _bx1, _by1, _bx2, _by2 = booth_raw
        _evm_w = max(0.0, _bx2 - _bx1)
        _evm_h = max(0.0, _by2 - _by1)
        _evm_roi = (
            max(0,          _bx1 - _evm_w * EVM_ROI_EXPAND_LR_SCALE),
            max(0,          _by1 - _evm_h * EVM_ROI_EXPAND_TOP_SCALE),
            min(frame_width,_bx2 + _evm_w * EVM_ROI_EXPAND_LR_SCALE),
            _by1 + (_by2 - _by1) / 2.0,
        )

    vat = cam_state.voter_area_threshold if cam_state else None
    person_classifications = []
    for box in person_dets.xyxy:
        is_voter, label = classify_person(box, booth_raw, vat)
        ax, ay, bx, by = box
        ar = max(1.0, by - ay) / max(1.0, bx - ax)
        # Override: centroid inside EVM ROI → always VOTER
        if _evm_roi is not None:
            cx = (ax + bx) / 2.0
            cy = (ay + by) / 2.0
            if (_evm_roi[0] <= cx <= _evm_roi[2] and
                    _evm_roi[1] <= cy <= _evm_roi[3]):
                is_voter = True
                label    = "VOTER"
        # During area learning, label as LEARNING so UI shows calibration state
        if is_learning_areas:
            label = "LEARNING"
        person_classifications.append({'is_voter': is_voter, 'label': label, 'ar': ar})

    # ── Staff Masking Logic ───────────────────────────────────────────────────
    voter_count = total_persons
    is_calibrating = False
    staff_indices = []
    
    if cam_state is not None:
        cam_state.cycle_frame_count += 1
        
        # Phase A: Learning
        if cam_state.cycle_frame_count <= STAFF_LEARNING_FRAMES:
            is_calibrating = True
            # Frame-level occupancy: mark cells covered by detections in this frame
            frame_occ = np.zeros((STAFF_GRID_SIZE, STAFF_GRID_SIZE), dtype=np.uint8)
            for box in person_dets.xyxy:
                # 1. Feet (bottom-center)
                gx_f, gy_f = get_grid_coords(box, frame_width, frame_height)
                y1_f = max(0, gy_f - 1); y2_f = min(STAFF_GRID_SIZE, gy_f + 2)
                x1_f = max(0, gx_f - 1); x2_f = min(STAFF_GRID_SIZE, gx_f + 2)
                frame_occ[y1_f:y2_f, x1_f:x2_f] = 1
                
                # 2. Centroid
                cx = (box[0] + box[2]) / 2.0; cy = (box[1] + box[3]) / 2.0
                gx_c = int((cx / frame_width) * STAFF_GRID_SIZE)
                gy_c = int((cy / frame_height) * STAFF_GRID_SIZE)
                y1_c = max(0, gy_c - 1); y2_c = min(STAFF_GRID_SIZE, gy_c + 2)
                x1_c = max(0, gx_c - 1); x2_c = min(STAFF_GRID_SIZE, gx_c + 2)
                frame_occ[y1_c:y2_c, x1_c:x2_c] = 1
            
            cam_state.occupancy_grid += frame_occ.astype(np.int32)
            
            if cam_state.cycle_frame_count == STAFF_LEARNING_FRAMES:
                # Generate base mask
                raw_mask = (cam_state.occupancy_grid >= STAFF_OCCUPANCY_THRESH).astype(np.uint8)
                # Dilation: expand staff cells by 1 to handle small movements
                kernel = np.ones((3, 3), np.uint8)
                cam_state.staff_mask = cv2.dilate(raw_mask, kernel, iterations=1).astype(int)
                log.info(f"[MASK] Generated dilated staff mask (cells={cam_state.staff_mask.sum()})")
    ...
    if booth_raw is not None and len(person_dets) > 0:
        # NEW: Keep using the old mask during calibration if it exists
        if cam_state is not None:
            # We only skip the mask if it's empty (all zeros), 
            # otherwise we keep using the old one while learning the new one.
            if cam_state.staff_mask.any():
                cur_mask = cam_state.staff_mask
                
        (behind_count, proximity_indices, overlap_info,
         primary_idx, secondary_info, radius_px,
         booth_expanded, dead_zone, buffer_px) = check_breach(
            person_dets, booth_raw, frame_width, frame_height, staff_mask=cur_mask, person_classifications=person_classifications)

    # ── Suppress Breach during First Calibration ──────────────────────────────
    if is_calibrating and (cur_mask is None or not cur_mask.any()):
        behind_count = 0
        proximity_indices = []
        secondary_info = []

    # ── Final Voter/Staff Refinement ──────────────────────────────────────────
    if cam_state is not None:
        voter_count = 0
        final_staff = []
        
        # Determine which mask to use for refinement
        refine_mask = None
        if cam_state.staff_mask.any():
            refine_mask = cam_state.staff_mask
            
        for i, box in enumerate(person_dets.xyxy):
            is_at_booth = False
            if booth_expanded is not None:
                is_at_booth, _, _ = person_at_booth(box, booth_expanded)
            
            if is_at_booth:
                voter_count += 1
            elif refine_mask is not None:
                gx, gy = get_grid_coords(box, frame_width, frame_height)
                if refine_mask[gy, gx] == 1:
                    final_staff.append(i)
                else:
                    voter_count += 1
            else:
                voter_count += 1
        staff_indices = final_staff

        # Reset Cycle
        if cam_state.cycle_frame_count >= STAFF_TOTAL_CYCLES:
            cam_state.occupancy_grid.fill(0)
            cam_state.cycle_frame_count = 0
    ...
    annotated = annotate_frame(
        ...
        overlap_info=overlap_info, radius_px=radius_px, buffer_px=buffer_px,
        vacant_active=(total_persons == 0),
        max_person_active=(voter_count > MAX_PERSON_THRESHOLD),
        proximity_indices=proximity_indices,
        voter_count=voter_count,
        is_calibrating=is_calibrating or is_learning_areas,
        staff_indices=staff_indices,
        person_classifications=person_classifications
    )
...
async def process_frames_multi(
    ...
    for (frame, vt), fut in zip(frames_with_times, futs):
        ...
        processed = process_single_frame_v3(
            ...
            cam_state=None # Multi-frame logic doesn't use staff masking yet or needs state ref
        )
...
async def video_coroutine_folder(
    ...
    try:
        while not shutdown.is_set():
            current_sec = cycle_idx * FOLDER_POLL_INTERVAL_SEC
            ...
            if frame is None:
                consecutive_fails += 1
                if duration_sec == 0 and consecutive_fails > 20:
                    break
                cycle_idx += 1
                ...
                if debug:
                    ...
                    if key == ord('q'):
                        ...
                        log.info(f"[MAIN] ⏭️ Skipping video: {cam_id}")
                        break
                continue
            ...
            if result["is_breach_frame"]:
                ...
                await loop.run_in_executor(upload_executor,
                    ...
                    result["annotated"], ts,
                    {"total": result["total"], "behind_count": result["behind"], "voters": result["voters"]},
                    "breach", None, cycle_idx + 1, cycle_stats, save_local=True))
            ...
            cv2.imshow(f"Processing: {cam_id}", result["annotated"])
            key = cv2.waitKey(1) & 0xFF
            if key == ord('q'):
                shutdown.set()
            elif key == ord('n'):
        ...
        cap.release()
        try:
            cv2.destroyWindow(f"Processing: {cam_id}")
        except Exception:
            pass
        log.info(f"[MAIN] ✅ Finished video: {cid if 'cid' in locals() else cam_id}")
...
async def async_folder_main(folder_path: str, debug: bool):
    ...
    while not shutdown.is_set():
        video_list = get_all_video_files(folder_path)
        # Natural sort: test1, test2, ..., test10, test11...
        video_list.sort(key=lambda x: [int(c) if c.isdigit() else c for c in re.split(r'(\d+)', x)])

        if not video_list:
            log.warning(f"[MAIN] No videos found in {folder_path}. Waiting 10s...")
        ...
        for i, vp in enumerate(video_list):
            ...
            log.info(f"[MAIN] 🎥 ({i+1}/{len(video_list)}) Processing: {cid}")
            await video_coroutine_folder(vp, cid, engine, grab_pool, upload_pool,
                                         shutdown, debug, cycle_stats)
        
        if shutdown.is_set():
            break
        log.info(f"[MAIN] 🏁 Folder cycle complete ({len(video_list)} videos). Re-scanning in 30s...")
...
class CameraState:
    def __init__(self):
        ...
        # Staff Masking State
        self.occupancy_grid     = np.zeros((STAFF_GRID_SIZE, STAFF_GRID_SIZE), dtype=np.int32)
        self.staff_mask         = np.zeros((STAFF_GRID_SIZE, STAFF_GRID_SIZE), dtype=np.int32)
        self.cycle_frame_count  = 0

        # Voter / officer area learning
        self.area_learning_frame_count  = 0
        self.collected_areas            = []
        self.voter_area_threshold       = None
        ...
        async def _process_frame_bg(frame=frame, h_px=h_px, w_px=w_px):
            ...
            if result["is_breach_frame"]:
                ...
                loop.run_in_executor(upload_executor, save_alert_image, cam_id,
                    ...
                    {"total": result["total"], "behind_count": result["behind"], "voters": result["voters"]},
                    "breach", None, cycle_stats.current_cycle, cycle_stats, True)
